...
export interface V10CommitAuthorizationFixture {
  ...
  issueAuthorizationRevocationReceipt(
 ...
 *
 * It issues real server-branded authorization evidence through Core's queued
 * Command path; a test-only persistence port stops before any durable write.
 */
export function createV10CommitAuthorizationFixture(): Readonly<V10CommitAuthorizationFixture> {
  ...
  const issueAuthorizationRevocationReceipt = async (
    intakeAuthorization: AuthorizedOfficeContext,
  ) => {
    const persistence: CommandLifecyclePersistencePort = {
      async readFinalReceipt() {
        return null;
      ...
      async commitAuthorizedCommand() {
        throw new Error(
          'V10_COMMIT_AUTHORIZATION_FIXTURE_UNEXPECTED_AUTHORIZED_COMMIT',
        );
      },
    };
    ...
    if (result.receipt.outcome !== 'AUTHORIZATION_REVOKED') {
      throw new Error(
        'V10_COMMIT_AUTHORIZATION_FIXTURE_REVOCATION_RECEIPT_MISSING',
      );
    }
