...
Numeric properties extend the Foundation shared arbitraries and
`packages/testkit/src/exact-decimal-oracle.ts`. Exact means canonical exact
equality; JS numbers, display rounding, epsilon and tolerance are forbidden.
...
| Inert canonical bytes | Getters, methods, duck providers, Proxy/hidden/symbol state     | Rejected before behavior executes; no trap/getter/method observation                                              | serialization reject              |
| Identity bridge       | UUID case/malformed subject and actor-ID substitution           | Canonical UUID AuthSubject resolves separately to current World actor/membership                                  | auth/schema reject                |
| Current authorization | Revoke/reassign/suspend/change revision after intake            | Protected approval/execution re-resolves and rejects; cached context has no authority                             | `PERMISSION_DENIED`, zero change  |
...
| Exact-or-reject closure | Boundary/carry/scale/overflow Money, Quantity and Price cases | Exact oracle match or explicit out-of-domain error; never rounded | deterministic domain reject     |
...
| Fail-closed startup       | Unsafe environment/loopback alias before Worker launch      | Exit before persistence client, lease, recovery or listener       | startup blocked              |
...
| Current settlement authority     | Revoke/reassign/change revision after acceptance        | Current protected step rejects; stale context/signature cannot authorize |
...
## Gate-A-to-Gate-B inherited regression invariants

These do not re-adjudicate Gate A. They prove World Core did not regress its
approved Foundation, if Gate A is later approved:

1. an unsafe runtime environment cannot start an authoritative Worker or touch
   persistence;
2. authoritative numeric operations remain exact-or-reject;
3. canonical serialization executes no getter, arbitrary method, duck provider
   or Proxy behavior;
4. protected authorization re-resolves current server state and treats cached
   context as non-authoritative;
5. `AuthSubject` remains a UUID-safe external identity distinct from World IDs;
6. forbidden import, ownership and numeric-coercion forms remain AST-blocked;
7. every authoritative migration proves immutable commit, path and exact
   artifact-byte provenance through the existing V02 chain.
...
9. all seven inherited Foundation regression invariants above.
