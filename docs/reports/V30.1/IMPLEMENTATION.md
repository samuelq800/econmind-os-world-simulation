...
counts, and **never** emits PASS. A new injected-executor runner dispatches a
plan with a hard 64-operation in-flight ceiling, abort/drain behavior, stable
observation ordering, and failure recording. It supplies no network adapter or
target credentials and cannot itself authorize a load test. It does not open a
socket, write a database, advance SimTime or mutate World State.

Focused V30.1 tests passed 10/10 across the plan and runner on this P2-only
mainline candidate. Targeted
...
The first standalone TypeScript invocation omitted `--types node` and failed
on the Node timers import; the corrected strict invocation passed.
