...
export const SINGLE_WORLD_CONFIGURATION_VERSION =
  'single-world-configuration-preparation-v2' as const;
...
export interface SingleWorldConfiguration {
  ...
  /** V27 country-set identity, not external configuration authority. */
...
export function prepareSingleWorldConfiguration(
  ...
  if (
    typeof record.countryConfigurationRef !== 'string' ||
    !SHA256.test(record.countryConfigurationRef)
  ) {
    invalid('Country configuration reference must be canonical SHA-256');
  }
  ...
  const configuration: SingleWorldConfiguration = Object.freeze({
    ...
    countryConfigurationRef: record.countryConfigurationRef as CanonicalSha256,
